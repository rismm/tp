...
##### **Saving**
Currently, there are only 2 commands that can make changes to
...
The string generated will have the attributes in the format and order of `NAME,QUANTITY,PRICE,TRANSACTION_DATE,TYPE`. This string will be appended to end the of the data file as mentioned.
...
currently in the program will be re-saved using string format above in the method `TransactionStorage-saveAllTransactions()`.
...
> Do note that the `end` is only a placeholder and can be any string of characters.
> > To justify why the saved string format is different from `Item` objects, the save file of `Transactions` can also act as a receipt of the user's expenses by making it much more readable.
...
> erased. e.g. If the file delimiter is determined to be `" ,,, "`, the item name `"app ,,, le` will be renamed to `"apple"`
